...
export default class MediaCaptionEditing extends Plugin {
  ...
  init() {
    ...
    editor.model.document.registerPostFixer((writer) =>
      this._insertMissingCaption(writer)
    );
    editor.conversion
      .for("upcast")
      .elementToElement({ model: "mediaCaption", view: matchMediaCaption });

    const createCaptionForData = (writer) =>
      writer.createContainerElement("figcaption");
    data.downcastDispatcher.on(
      "insert:mediaCaption",
      captionModelToView(createCaptionForData, false)
    );

    const createCaptionForEditing = captionElementCreator(
      view,
      t("Enter media caption")
    );
    editing.downcastDispatcher.on(
      "insert:mediaCaption",
      captionModelToView(createCaptionForEditing)
    );
    editing.downcastDispatcher.on(
      "insert",
    ...
    editing.downcastDispatcher.on(
      "remove",
      this._fixCaptionVisibility((data) => data.position.parent),
      { priority: "high" }
    );
    view.document.registerPostFixer((writer) =>
      this._updateCaptionVisibility(writer)
    );
  }
  ...
  _insertMissingCaption(writer) {
    ...
    for (const media of missing) {
      writer.appendElement("mediaCaption", media);
    }
...
function captionModelToView(elementCreator, hide = true) {
  return (evt, data, conversionApi) => {
    ...
    if (isMedia(captionElement.parent)) {
      ...
      const viewMedia = conversionApi.mapper.toViewElement(
 ...
 */
function insertViewCaptionAndBind(
  viewCaption,
  modelCaption,
  viewMedia,
  conversionApi
...
function getParentCaption(node) {
  ...
  const caption = ancestors.find(
    (ancestor) => ancestor.name === "mediaCaption"
  );

  return caption && caption.parent && caption.parent.name === "media"
    ? caption
    : null;
}
