...
export default class MediaEditing extends Plugin {
  ...
  init() {
    ...
    conversion.for("editingDowncast").elementToElement({
      ...
      view: (element, writer) =>
        toMediaWidget(
          createMediaViewElement(element, writer),
          writer,
          t("Media Widget")
        ),
    });
    ...
    ["alt", "height", "src", "width"].forEach((attr) =>
      conversion.for("downcast").add(modelToViewAttribute(attr))
    );
  }
}
...
function createMediaViewElement(element, writer) {
  ...
  if (type.id === "image") {
    ...
    media = writer.createContainerElement(type.element, {
      controls: "controls",
    });
