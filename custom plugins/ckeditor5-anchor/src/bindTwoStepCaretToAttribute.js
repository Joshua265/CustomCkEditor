...
export default function bindTwoStepCaretToAttribute({
  ...
  const twoStepCaretHandler = new TwoStepCaretHandler(
    model,
    emitter,
    attribute
  );
  ...
  emitter.listenTo(
    ...
    (evt, data) => {
      ...
      if (
        (contentDirection === "ltr" && arrowRightPressed) ||
        (contentDirection === "rtl" && arrowLeftPressed)
      ) {
        isMovementHandled = twoStepCaretHandler.handleForwardMovement(
          position,
          data
        );
      } else {
        isMovementHandled = twoStepCaretHandler.handleBackwardMovement(
          position,
          data
        );
      }
...
export class TwoStepCaretHandler {
  ...
  constructor(model, emitter, attribute) {
    ...
    emitter.listenTo(this._modelSelection, "change:range", (evt, data) => {
      ...
      if (
        !data.directChange &&
        isAtBoundary(this._modelSelection.getFirstPosition(), attribute)
      ) {
        return;
      }
   ...
   */
  _overrideGravity() {
    this._overrideUid = this.model.change((writer) =>
  ...
  _setSelectionAttributeFromTheNodeBefore(position) {
    ...
    this.model.change((writer) => {
      writer.setSelectionAttribute(
        this.attribute,
        position.nodeBefore.getAttribute(attribute)
      );
    });
  }
...
function isAtBoundary(position, attribute) {
  return (
    isAtStartBoundary(position, attribute) ||
